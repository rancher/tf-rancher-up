...
variable "cloud_credential_id" {
  description = "Existing cloud credential ID. If not provided, a new credential will be created"
  type        = string
  default     = null
}
...
variable "cluster_description" {
  ...
  type        = string
  default     = "Terraform managed GKE cluster"
}
...
variable "region" {
  ...
  validation {
    ...
    error_message = "The region must be a valid GCP region or zone. Please refer to https://cloud.google.com/about/locations for available regions and zones."
  }
}
...
variable "cloud_credential_name" {
  description = "Name for the cloud credential (only used when creating new credential)"
  type        = string
  default     = "terraform-gcp-credential"
}
