...
variable "vsphere_folder" {
  type        = string
  description = "vSphere folder for VM placement (optional)"
  default     = null
}
...
variable "instance_count" {
  type        = number
  description = "Number of K3s server nodes (use odd numbers: 1, 3, 5 for HA)"
  default     = 3
...
variable "k3s_version" {
  type        = string
  description = "K3s version to use. Leave null for the latest in a channel."
  default     = null
}

variable "k3s_channel" {
  type        = string
  description = "K3s channel to use (default: stable)"
  default     = "stable"
}

variable "k3s_token" {
  type        = string
  description = "K3s cluster token (auto-generated if null)"
  sensitive   = true
  default     = null
}
...
variable "k3s_config" {
  type        = string
  description = "Additional K3s config.yaml content (YAML format)"
  default     = null
}

variable "k3s_ingress" {
  type        = string
  description = "Ingress controller mapped to Rancher values (default traefik for K3s usually, but leaving flexibility if modified through k3s_config)"
  default     = "traefik"

  validation {
    condition     = contains(["nginx", "traefik", "ingress-nginx"], var.k3s_ingress)
    error_message = "k3s_ingress must be 'nginx', 'traefik', or 'ingress-nginx'"
  }
}
